...
import { Dialog, Column } from '@/once-ui/components'
...
export const MyOverlay = ({ index, src, aspect_ratio }: { index: number, src: string, aspect_ratio: number }) => {
    const ovl = useOverlay()
    return (
        <>
        {/* <Column width="xl" height="xl" > */}
            <Dialog
                isOpen={ovl.selected[index]}
                onClose={() => ovl.setSelected(index, !ovl.selected[index])}
                title="Look at the pretty picture"
                description="Wow its so pretty"
                // position="absolute"
                style={{ width: "100%", height: "100%", position: "absolute" }}
            >
                <Column style={{ width: "100%", height: "100%"}} justifyContent="center" alignItems="center">
                    <Image src={src} alt="robday" width={1000 * aspect_ratio} height={1000} />
                </Column>
            </Dialog>
            {/* </Column> */}
